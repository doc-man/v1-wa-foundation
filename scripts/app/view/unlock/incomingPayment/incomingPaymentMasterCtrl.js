'use strict';
angular.module('myApp').controller("incomingPaymentMasterCtrl", ['$scope', '$state', '$stateParams', 'unlockService', 'userServices', '$filter',
    function($scope, $state, $stateParams, unlockService, userServices, $filter) 
    {
        $scope.nameToShowInBreadcrumb = "Unlock / incomingPayment";
        ...
        {
            if(prName == 'incomingPayments')
            {
                $scope.eventIDPayments = true;
                unlockService.unlock('showIncomingPayments').showIncomingPayments({id: $scope.unlockData.eventID}, function(data)
                {
                    $scope.incomingPaymentList = data.data;
                });
            }
        };
        $scope.fnSaveUnlockedEvent = function(prName) 
        {
            ...
            $scope.unlockData.newValue = 'Incoming payment : '+$scope.unlockData.paymentID.amountClaimed+' unlocked, Reason: '+$scope.unlockData.reason;
            ...
            unlockService.unlock('unlockPayment').unlockPayment({}, $scope.unlockData, function(data)
            {
                ...
                {
                    alert('Incoming payment:'+$scope.unlockData.paymentID.amountClaimed+' unlocked successfully.')
                    $scope.unlockData = {};
                    $scope.unlockDetailName = 'List';
                ...
                {
                    alert('Incoming payment:'+$scope.unlockData.paymentID.amountClaimed+' not found.')
                    $scope.unlockDetailConfirm = 'No';
                }
            });
        };
