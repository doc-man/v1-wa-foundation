'use strict';
angular.module('myApp').controller("eventMasterCtrl", ['$scope', '$state', '$stateParams', 'unlockService', 'userServices', '$filter',
    function($scope, $state, $stateParams, unlockService, userServices, $filter) 
    {
        $scope.nameToShowInBreadcrumb = "Unlock / Event";
        ...
        $scope.fnSaveUnlockedEvent = function(prName) 
        {
            ...
            $scope.unlockData.nameOfClient = 'MasterDB';
            $scope.unlockData.currentDateTimeOfClient = localDateTime;
            $scope.unlockData.timeZoneOfClient = localTimeZoneName;
            $scope.unlockData.idOfEventOnWhichAcitivtyIsDone = $scope.unlockData.eventID;
            unlockService.unlock('unlockEvent').unlockEvent({}, $scope.unlockData, function(data)
            {
                if(data.data=="success")
                {
                    alert('Event id:'+$scope.unlockData.eventID+' unlocked successfully.')
                    $scope.unlockData = {};
                    $scope.unlockDetailName = 'List';
                    $scope.unlockDetailConfirm = 'No';
                }
                ...
                {
                    alert('Event id:'+$scope.unlockData.eventID+' not found.')
                    $scope.unlockDetailConfirm = 'No';
                }
            });
        };
